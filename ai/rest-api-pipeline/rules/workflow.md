...
3. **Add pipeline MCP** (`add-pipeline-mcp`) — configure MCP server so the agent can query loaded data via SQL
4. **Validate data** (`validate-data`) — inspect schema and data, fix types and structures, iterate until user is satisfied
...
- `debug-pipeline` → if pipeline loads successfully, use `add-pipeline-mcp` to configure MCP, then move to `validate-data`
